...
const ChatPanel = () => {
  ...
  const formatTime = (dateStr: string) =>
    new Intl.DateTimeFormat("vi-VN", { hour: "numeric", minute: "numeric" }).format(new Date(dateStr));
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
        {sorted.length === 0 ? (
          ...
          sorted.map((conv) => {
            ...
            const displayName = isGroup ? conv.name ?? "Nhóm không tên" : other?.displayName ?? "Unknown";
            const avatarUrl = other?.avatarUrl;
            const online = !isGroup && other ? onlineUserIds.includes(other._id) : false;
            ...
            const stackAvatars = (others.length > 0 ? others : participantObjects).slice(0, 2);
            ...
            return (
              ...
              >
                ...
                <div className="relative shrink-0">
                      ...
                      {stackAvatars.length === 0 ? (
                        ...
                        <div className="flex -space-x-2.5">
                          {stackAvatars.map((p, i) => (
                            <Avatar key={p._id} className={`h-7 w-7 border-2 border-background ${i === 0 ? "z-20" : "z-10"}`}>
                  ...
                  {/* Online dot for DMs */}
                  {!isGroup && (
                    <span className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-background ${online ? "bg-green-500" : "bg-muted-foreground/30"}`} />
                  )}
                </div>
                ...
                <div className="flex flex-col overflow-hidden min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-1">
                    <span className={`font-bold text-[15px] leading-tight truncate ${isActive ? "text-[#2dd4bf]" : "text-foreground"}`}>
                      {displayName}
                    </span>
